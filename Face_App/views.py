...
def Attendance(request):
	if request.method == "POST":
		latitude = request.POST.get('text1')
		longitude = request.POST.get('text2')
		...
		# Define the center location (latitude and longitude)
		center_location = (19.164628, 72.849354)  # San Francisco, CA

		# Define the range (in meters)
		range_in_meters = 300  # 1 kilometer

		# Define another location to check (latitude and longitude)
		another_location = (latitude, longitude)

		# Calculate the distance between the center location and the another location
		distance = geodesic(center_location, another_location).meters

		# Check if the distance is within the defined range
		if distance <= range_in_meters:
			print("The another location is within the range of the center location.")
		    ...
		    print("The another location is outside the range of the center location.")
		    return JsonResponse({'data': 'Out of range'})
...
def View_Attendance_User(request):
	...
	data = userDetails.objects.get(username = C_name)
	print(C_name)
	name = data.F_name
	print("Users Name :",name)
	details = attendance.objects.filter(name=name)
